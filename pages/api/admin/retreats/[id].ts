  ...
  try {
    if (req.method === "GET") {
      ...
      return res.status(200).json({
        ...
        startDate: (entry.fields.startDate as any)?.["en-US"] ?? "",
        endDate: (entry.fields.endDate as any)?.["en-US"] ?? "",
    ...
    if (req.method === "PUT") {
      const { title, slug, startDate, endDate, summary, content, assetId } = req.body;
